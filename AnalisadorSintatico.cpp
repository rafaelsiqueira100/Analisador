...
void AnalisadorSintatico::CompProcedimento() throw()
{
    ...
    this->nivelAtual--;
    //comando composto jÃ¡ vai incrementar nivelAtual
    ...
    while(prox == AbreParenteses || prox==Subtracao){
        ...
        while(prox == FechaParenteses){
            if(numParenteses==0 && this->NaoHaErro()){
                this->erro = "ParÃªnteses desbalanceados!";
                cout << '\n'<<"Linha atual :"<< this->anaLex->getLinhaAtual() << '\n'<< this->erro;
                return;
            }
            numParenteses--;
        ...
        while(numParenteses>parentesesNivel || EhOperadorAritmetico(prox)){
			...
			{
                this->erro = "Operador aritmÃ©tico esperado!";
                cout << '\n'<<"Linha atual :"<< this->anaLex->getLinhaAtual() << '\n'<< this->erro;
                return;
            }
        while(prox == AbreParenteses || prox==Subtracao){
...
void AnalisadorSintatico::CompOperandoBooleano()throw(){
    ...
    if (prox == Identificador || prox==Numero)
    {
        ...
        {//com certeza Ã© expressao relacional
            if(prox==Identificador){
                if(EhIdDeVariavel(id))
                {
                    CompChamadaDeVariavel();
...
void AnalisadorSintatico::CompDeclaracaoVariavel() throw()
{
    ...
    if ((!this->EhStringAlfabetico(id)) && this->NaoHaErro())
    {
        ...
        anaLex->proximoPedaco(); //Consome a parte alfabética do identificador
...
//CompChamadaDeFuncao consome ; se for retorno da prÃ³pria funcao
//se for sÃ³ uma chamada de fora da funcao, nao consome ;
...
foraLoop:
    ...
    this->nivelAtual--;
    //comando composto jÃ¡ vai incrementar nivel atual
    CompComandoComposto();
