...
void AnalisadorSintatico::CompDeclaracaoVariavel() throw()
{
    ...
    if (prox == Logico || prox == Inteiro)
    {
        ...
        for (; i < tamanhoLista; i++)
        {
            ...
            Variavel* var=new Variavel(id, this->nivelAtual, tipoVar);

            this->tabela.guarde(var);
            lista.pop_front();
        }
...
void AnalisadorSintatico::CompExpressaoRelacional() throw()
{
    ...
    std::list<char> lista();
    std::list<TipoPedaco> listaTipos();
    std::list<string> listaLiterais();
    while(!lista.empty()){
        while(prox==AbreParenteses){
            lista.push_back(*(anaLex->getLiteral().c_str()));
            anaLex->proximoPedaco();
            prox = anaLex->verPedaco();
        }
        char retorno = EhExpressao(&lista, &listaLiterais);
        if(primeiraVez)
            compBool = EhBool(*(listaLiterais.begin()));
        else{
            if(compBool){
                switch(retorno){
                    case 0:
                        if(!EhBool(*(listaLiterais.begin()))){
                            this->erro = "Conflito de tipo!"
                            cout <<'\n'<<this->erro;
                            return;
                        }
                        CompChamadaDeVariavel();
...
/*
        retorno
        0> não é expressão
        1> é expressão aritmética
        2> é expressão lógica
        3>é expressão reacional
        */
        /*if(prox == Identificador){
            string id = anaLex->getLiteral();

            Simbolo* simbolo;

                if(!this->tabela.encontrar(id,simbolo )){
                    this->erro = "Identificador não declarado";
                    cout <<'\n' <<this->erro ;
                    return;
                }

                if(primeiraVez)
                    compBool = EhBool(*simbolo);
                else{
                    if(EhBool(*simbolo)!= compBool){
                        this->erro = "Comparação de tipos diferentes!";
                        cout << '\n'<<this->erro;
                        return;
                    }
                }
...
void AnalisadorSintatico::CompDeclaracaoVariavel() throw()
{
    ...
    if (prox == Logico || prox == Inteiro)
    {
        ...
        {
            id = *(lista.begin());
            Variavel* var=new(id, this->nivelAtual, tipoVar);
            this->tabela.guarde(var);
            lista.pop_front();
        }
        lista.erase(lista.begin(), lista.end());
